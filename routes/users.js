...
router.post("/favoriteGames", async (req, res, next) => {
  try {
    ...
    res.status(201).send("The game was successfully saved as favorite");
  } catch (error) {
    next(error);
  }
});
...
router.get("/favoriteGames", async (req, res, next) => {
  try {
    const user_id = req.session.user_id;
    const games = await users_utils.getFavoriteGames(user_id);
    if(games.length==0)
      res.status(204).send("no favorite games to this user");
    else
